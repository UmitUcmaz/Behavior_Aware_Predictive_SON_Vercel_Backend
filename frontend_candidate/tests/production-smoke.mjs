...
const backend = process.env.BAPS_BACKEND_URL;
const backendBypass = process.env.BAPS_BACKEND_BYPASS?.trim();
...
async function request(path, options = {}) {
  ...
  const direct = path.startsWith('/direct/');
  headers.set('x-vercel-protection-bypass', direct ? backendBypass : bypass);
  const url = direct ? `${backend}/api/${path.slice(8)}` : `${base}${path}`;
...
try {
  const health = await json('/direct/health');
  assert.equal(health.status, 'ok');
  record('backend-health', { status: 'PASS', engine: health.engine });
  const generation = await sse('generation-sse', '/direct/dataset-generator/generate/events?cell_count=100&seed=42');
  ...
  const forecast = await sse('forecast-sse', `/direct/forecast/generated/events?generation_id=${encodeURIComponent(generation.generation_id)}&horizon_steps=96`);
  ...
  for (const prefix of ['/direct/','/api/backend/']) {
  ...
  await writeFile(join(tmpdir(),'baps-production-smoke-report.json'),JSON.stringify(report,null,2));
}
